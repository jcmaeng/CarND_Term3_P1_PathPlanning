...
int main() {
  ...
  // start at curr_lane 1
  int curr_lane = 1;

  // reference velocity of my car
  double ref_speed = 0.0; // mph

  h.onMessage([&ref_speed, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &curr_lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for (int i = 0; i < sensor_fusion.size(); i++)
            {
              ...
              bool detected_at_left_lane = false;
              if (detected_d > (2 + 4 * (curr_lane - 1) - 2) &&  detected_d < (2 + 4 * (curr_lane - 1) + 2))
                detected_at_left_lane = true;

              bool detected_at_right_lane = false;
              if (detected_d > (2 + 4 * (curr_lane + 1) - 2) && detected_d < (2 + 4 * (curr_lane + 1) + 2))
                detected_at_right_lane = true;

              bool need_distance = false;
              if (abs(detected_s - car_s) < SAFE_DISTANCE)
                need_distance = true;

              bool detected_at_front = false;
              if (detected_s > car_s)
            ...
            if (decrease_speed)
            {
              ...
              //change to left lane
              if (curr_lane > 0 && left_is_empty)
              {
                curr_lane--;
              }
              //change to right lane
              else if (curr_lane < 2 && right_is_empty)
              {
                curr_lane++;
              }
            }
            ...
            double N = (target_dist / (TIME_INTERVAL * ref_speed * MPH_TO_MPS));
